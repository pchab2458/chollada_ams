...
@login_required
def gateway(request):
    if str(request.user) in ['Admin Admin', 'Preecha Bootwicha','Teerawut Bootwicha']:
...
# -------------- TEST 2 final--------------------------------------------------------------
    ...
    if sdate.year == bdate.year:
        ...
        if (start_m == 12) and (sdate.year + 1 == bdate.year) and (bill_m == 1) and (start_day >= bill_day):  # DECEMBER/YEAR CHANGE
